...
from db.models import Element, BondType
from pydantic import BaseModel
from typing import List, Dict, Any


class ElementModel(BaseModel):
    """Модель химического элемента"""
    symbol: str
    name: str
    valence: int
    color: str
...
class BondTypeModel(BaseModel):
    """Модель типа химической связи"""
    label: str
    value: int
    color: str
    style: str
...
class ConstantsResponse(BaseModel):
    """Модель ответа с константами системы"""
    periodic_table: List[ElementModel]
    bond_types: List[BondTypeModel]
...
@router.get("/api/constants",
            ...
            summary="Получение констант системы",
            description="Возвращает периодическую таблицу элементов и типы химических связей, используемые в системе.",
            responses={
                200: {"description": "Константы системы успешно получены"}
            })
